    ...
    <el-main class="cmp-admin-content-container">
      <div class="cmp-admin-content">
            ...
            <div class="restaurant-details">
              ...
              <el-form label-width="150px">
                ...
                <el-form-item label="CNPJ" class="cmp-admin-form-item">
                  <el-input v-model="restaurantCNPJ" class="cmp-admin-form-input" v-mask="'##.###.###/####-##'"
                    @input="validateCNPJ" required></el-input>
                </el-form-item>
                ...
                <el-form-item label="Endereço" class="cmp-admin-form-item">
                  <div class="cmp-admin-flex">
                    ...
                    <el-input v-model="restaurantAddress.district" placeholder="Bairro" class="cmp-admin-form-input"
                      required></el-input>
                  </div>
                  <div class="cmp-admin-flex">
                    <el-input v-model="restaurantAddress.street" placeholder="Nome da Rua" class="cmp-admin-form-input"
                      required></el-input>
                    <el-input v-model="restaurantAddress.streetNumber" placeholder="Número" class="cmp-admin-form-input"
                      required></el-input>
                  </div>
                  <div class="cmp-admin-flex">
                    <el-input v-model="restaurantAddress.city" placeholder="Cidade" class="cmp-admin-form-input"
                      required></el-input>
                    <el-input v-model="restaurantAddress.state" placeholder="Estado" class="cmp-admin-form-input"
                      required></el-input>
                  </div>
                </el-form-item>
                ...
                <el-form-item label="Descrição" class="cmp-admin-form-item">
                  <el-input type="textarea" v-model="restaurantDescription" class="cmp-admin-form-textarea" required
                    placeholder="Breve resumo"></el-input>
                </el-form-item>
              </el-form>
            </div>
        ...
        <div v-else-if="activeTab === 'pedido'">
          ...
          <PedidoStage v-for="(pedidos, index) in stages" :key="index" :stage="pedidos.stage"
            :stageTitle="pedidos.stageTitle" :pedidos="pedidos.pedidos" />
        </div>
          ...
          <div class="cmp-admin_addmenu-items">
            <el-card v-for="(prato, index) in pratos" :key="index" class="cmp-admin_addmenu-item">
              <div class="cmp-admin_addmenu-function">
                ...
                <el-button class="cmp-admin_addmenu-function-btn"
                  @click="removerPrato(prato.id, index)">Remover</el-button>
              </div>
        ...
        <div v-else-if="activeTab === 'ajuda'">
          ...
          <div class="restaurant-details">
            <p class="cmp-admin-restaurant-information cmp-admin-spacebottom">Algumas das principais dúvidas
              sobre informações do sistema.</p>
          </div>
          <el-collapse v-model="faqAccordion">
            <el-collapse-item class="cmp-admin_collapse-title" title="O que são as categorias ?" name="faq">
              <div class="faq-content cmp-admin_collapse-text">
                <p>Tipos de alimentos referem-se a classificações ou grupos nos quais os alimentos podem ser
                  organizados com base em diferentes critérios. Essas categorias podem ser variadas e
                  servem para auxiliar na identificação, classificação e seleção de alimentos de acordo
                  com suas características específicas. Alguns exemplos comuns de categorias de tipos de
                  alimentos incluem:
                </p>
                <ul class="cmp-admin_collapse-list">
                  <li><strong>Sem Glúten:</strong> Refere-se a alimentos que não contêm glúten, uma
                    proteína encontrada em cereais como trigo, cevada e centeio. Esses alimentos são
                    adequados para pessoas com intolerância ao glúten ou doença celíaca.</li>
                  <li><strong>Sem Lactose:</strong> Este grupo consiste em alimentos que não contêm
                    lactose, o açúcar natural encontrado nos produtos lácteos. Isso inclui uma variedade
                    de alternativas livres de laticínios, como leites vegetais (amêndoa, coco, arroz),
                    queijos e iogurtes sem lactose. Esses alimentos são ideais para pessoas com
                    intolerância à lactose, que têm dificuldade em digerir esse açúcar, resultando em
                    desconforto gastrointestinal após consumir produtos lácteos tradicionais, como
                    leite, queijo ou iogurte.</li>
                  <li><strong>Alimentos Vegetarianos:</strong> Os alimentos vegetarianos excluem carne,
                    aves e peixes, mas incluem produtos lácteos e ovos, sendo baseados em fontes
                    vegetais como frutas, legumes, grãos, leguminosas, nozes e sementes.</li>
                  <li><strong>Alimentos Integrais:</strong> Refere-se a alimentos não processados ou
                    minimamente processados, que mantêm a maior parte de sua forma original, como grãos
                    integrais, legumes frescos, etc.</li>
                  <li><strong>Alimentos Veganos:</strong> Os alimentos veganos eliminam completamente
                    qualquer produto de origem animal, incluindo laticínios, ovos e mel. Eles consistem
                    apenas em alimentos de fontes vegetais, como frutas, verduras, legumes, grãos,
                    leguminosas, nozes e sementes.</li>
                </ul>
              </div>
            </el-collapse-item>
          </el-collapse>
...
export default {
  ...
  mounted() {
    ...
    this.fetchRestaurantesDoUsuario();
    this.fetchPratos();
