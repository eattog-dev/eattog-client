...
export default {
    ...
    methods: {
        ...
        async fetchRestaurantes() {
            try {
                const response = await axios.get('https://api.eattog.jera.com.br/meus-restaurantes', {
                headers: { 'Authorization': sessionStorage.getItem('token-admin') }
                });

                const restaurantes = response.data.restaurante || []; 
                console.log("restaurantes", restaurantes);
                if (restaurantes.length > 0) {
                    const primeiroRestauranteId = restaurantes[0].id;
                    console.log("primeiroRestauranteId", primeiroRestauranteId);
                    sessionStorage.setItem('restaurante-id', primeiroRestauranteId);
                }
            } catch (error) {
                console.error('Erro ao buscar informações dos restaurantes:', error);
            }
        },
        ...
        adicionarNovoPrato() {
            ...
            console.log("this.novoPrato.ingredientes", this.novoPrato.ingredientes);
            console.log("this.novoPrato.tempo_preparo", this.novoPrato.tempo_preparo);
            console.log("this.novoPrato.descricao", this.novoPrato.descricao);
            console.log("this.novoPrato.categoria_prato", this.novoPrato.categoria_prato);
            console.log("this.novoPrato.desconto", this.novoPrato.desconto);
            console.log("this.novoPrato.valor_desconto", this.novoPrato.valor_desconto);
